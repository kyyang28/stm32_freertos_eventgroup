...
#define DATAPROCESS_STK_SIZE                        1024
...
#define LED6ON                                      5
#define LED6OFF                                     6
#define COMMANDER                                   0xFF        // invalid command

/* Convert lower-case letter to upper-case letter */
void LowerToUpper(uint8_t *str, uint8_t len)
{
    uint8_t i;
...
/* Manipulate command values */
uint8_t CommandProcess(char *str)
{
    uint8_t CommandValue = COMMANDER;
    
//    printf("str: %s, %s\r\n", str, __FUNCTION__);
    
    if (strcmp(str, "LED4ON") == 0) {
//        printf("str: %s, %s\r\n", str, __FUNCTION__);
        CommandValue = LED4ON;
    } else if (!strcmp((char *)str, "LED4OFF")) {
        CommandValue = LED4OFF;
    } else if (!strcmp((char *)str, "LED5ON")) {
        CommandValue = LED5ON;
    } else if (!strcmp((char *)str, "LED5OFF")) {
        CommandValue = LED5OFF;
    } else if (!strcmp((char *)str, "LED6ON")) {
        CommandValue = LED6ON;
    } else if (!strcmp((char *)str, "LED6OFF")) {
        CommandValue = LED6OFF;
    }
...
//                printf("%s, %d\r\n", __FUNCTION__, __LINE__);
                ...
                /* Copy the content of the UART buffer to CommandStr array */
//                while ((ch = rxSerial3TestRead()) != 0x0A) {
//                    CommandStr[i++] = ch;
//                }

//                for (int i = 0; i < strlen((char *)s->port.rxBuffer); i++) {
//                    printf("s->port.rxBuffer[%d] = %c\r\n", i, s->port.rxBuffer[i]);
//                }

                sprintf((char *)CommandStr, "%s", (char *)s->port.rxBuffer);

//                printf("RecvLen: %d\r\n", RecvLen);
                
//                CommandStr[i-2] = '\0';           // eliminate the 0x0D('\r') and 0x0A('\n') characters
                CommandStr[RecvLen-2] = '\0';           // eliminate the 0x0D('\r') and 0x0A('\n') characters
                
//                for (int i = 0; i < strlen(CommandStr); i++) {
//                    printf("CommandStr[%d] = %c\r\n", i, CommandStr[i]);
//                }
                
//                printf("CommandStr: %s\r\n", CommandStr);

                /* Convert lower-case letters to upper-case letters */
                LowerToUpper(CommandStr, RecvLen);
                
//                printf("CommandStr: %s\r\n", CommandStr);
                
                /* Call CommandProcess() function to parse the command value */
                CommandValue = CommandProcess(CommandStr);
                
//                printf("CommandValue: %d\r\n", CommandValue);
                
                /* Perform the corresponding operations based on the command values */
                if (CommandValue != COMMANDER) {
//                    printf("CommandStr: %s\r\n", CommandStr);
                    switch (CommandValue) {
                        case LED4ON:
                            LED4_ON;
                ...
                /* Clear CommandStr buffer */
                RecvLen = 0;                        // Reset the rxBuffer length
//                memset(s->port.rxBuffer, 0, sizeof(s->port.rxBuffer));
//                memset(CommandStr, 0, sizeof(CommandStr));
            }
        } else if (err == pdFALSE) {
            vTaskDelay(10);
