...
#include "event_groups.h"   // event groups operations
#endif
...
void eventgroup_task(void *pvParameters)
{
    ...
    while (1) {
        if (EventGroupHandler != NULL) {
            ...
            EventValue = xEventGroupWaitBits(EventGroupHandler, EVENTBIT_0, pdTRUE, pdTRUE, portMAX_DELAY);
            
            printf("Event value: %d, %s\r\n", EventValue, __FUNCTION__);
...
void eventquery_task(void *pvParameters)
{
    ...
    while (1) {
        if (EventGroupHandler != NULL) {
            NewValue = xEventGroupGetBits(EventGroupHandler);           // Acquire event group bits
            
            if (NewValue != LastValue) {
                LastValue = NewValue;
                printf("Event value: %d, %s\r\n", NewValue, __FUNCTION__);
            }
        }
